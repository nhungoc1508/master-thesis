...
def main():
    ...
    if args.ckpt: # ----- eval-only (region/domain transfer) -----
        ...
        model = Seq2Seq(a['rnn'], a['d_model'], a['layers']).to(device)
        model.load_state_dict(ck['model'])
        logger.info('Loaded checkpoint %s (trained rnn=%s task=%s)', args.ckpt, a.get('rnn'), a.get('task'))
    else: # ----- train -----
        ...
        if args.save_ckpt:
            common.save_ckpt(args.save_ckpt, {'model': model.state_dict(),
                                              'arch': dict(rnn=args.rnn, d_model=args.d_model,
                                                           layers=args.layers, task=args.task)})
