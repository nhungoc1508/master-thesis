"""
Minimal changes to adapt to newer numpy versions
    - np.int -> np.int64
    - np.array(x) -> np.array(x, dtype=object)
All changes marked with [np-compat]
"""
import numpy as np
import torch
import constants
...
def pad_arrays(a):
    ...
    a = np.stack(a).astype(np.int64) # [numpy-compat]
...
def pad_arrays_pair(src, trg, keep_invp=False):
    ...
    idx = argsort(src)
    src = list(np.array(src, dtype=object)[idx]) # [numpy-compat]
    trg = list(np.array(trg, dtype=object)[idx]) # [numpy-compat]
