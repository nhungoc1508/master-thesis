"""
Inspect .parquet files and report:
    - Number of GPS points
    - Number of trajectories

Usage:
    python scripts/inspect_parquet_stats.py data/canonical/
...
def main():
    ...
    data_dir_relative = Path(args.dir)
    data_dir = _PROJECT_ROOT / data_dir_relative
    ...
    out_path = logs_dir / f'stats_{data_dir_relative.stem}_{ts}.json'
    out_path.write_text(json.dumps(output, indent=4))

    logger.info('Written to %s', out_path)
    logger.info(f'Total: {total_points:,} points across {total_trajectories:,} trajectories')
